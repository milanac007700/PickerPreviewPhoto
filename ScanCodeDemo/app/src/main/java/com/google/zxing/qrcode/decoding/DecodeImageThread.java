    ...
    public void run() {
        if (null == mData) {
            if (!TextUtils.isEmpty(mImgPath)) {
                Bitmap bitmap = QRCodeUtil.decodeSampledBitmapFromFile(mImgPath, MAX_PICTURE_PIXEL, MAX_PICTURE_PIXEL);
                this.mData = QRCodeUtil.getYUV420sp(bitmap.getWidth(), bitmap.getHeight(), bitmap);
        ...
        final Result result = QRCodeUtil.decodeImage(mData, mWidth, mHeight);
